...
namespace Client
{
    ...
    {
        protected CharBase      _TargetChar    = null; // 기능 타겟 캐릭터 기능은 타겟이 가지고있다.
        ...
        public virtual void RunExecution(bool StartExecution)
        {
            if (StartExecution)
            {
                if (!_TargetChar.ExecutionBaseDic[_ExecutionData.GroupType].Contains(this))
                {
                    _StartTime = Time.time;
                    _TargetChar.ExecutionBaseDic[_ExecutionData.GroupType].Add(this);
                }
            }
            else 
            {
                if(_TargetChar.ExecutionBaseDic[_ExecutionData.GroupType].Contains(this))
                {
                    _TargetChar.ExecutionBaseDic[_ExecutionData.GroupType].Remove(this);
                }
            }
        }
        ...
        public void CheckTimeOver()
        {
            ...
            float executionTime = _ExecutionData.ExecutionTime/ SystemConst.Persent;
